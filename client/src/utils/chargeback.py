...
class LicensePluginRemote(RemoteBasePlugin):
    ...
    def query(self, **kwargs):
        ...
        if os.path.isfile(f'{self.tempfile}') and os.path.getsize(f'{self.tempfile}'):
            ...
            cache["hosts"] = {}
            self.last_millis = current_millis
        time_elapsed = current_millis - self.last_millis
        if time_elapsed > 24 * 60 * 60 * 1000: # Cap it at 24 hours to hopefully not run out of time executing the API calls
            ...
            self.last_millis = current_millis - 24 * 60 * 60 * 1000
        self.logger.info(f"Current milliseconds: {current_millis}")
        ...
        if time_elapsed >= 59 * 60 * 1000 and now.minute == 0: # Send data once per hour
            resetStats = {}
            resetStats["last_millis"] = current_millis
            resetStats["hosts"] = {}
            ...
            entity_definitions = {}
            self.calculate_and_push_consumption_for_dem(entity_definitions, current_millis)
            self.logger.info(f"Calculating DDU...")
            self.calculate_and_push_consumption_for_ddu(entity_definitions, current_millis, cache["hosts"])
            self.logger.info(f"Pushing HU and HU hours...")
            self.push_consumption_for_host_units(current_millis, cache["hosts"])
            ...
            self.get_consumption_for_host_units(current_millis, cache["hosts"])
    ...
    def get_consumption_for_host_units(self, current_millis, hosts):
        nextPageKey = ""
    ...
    def push_consumption_for_host_units(self, current_millis, hosts):
        payload = ''
        number_of_lines = 0
        for host in hosts:
    ...
    def calculate_and_push_consumption_for_dem(self, dem_entities_values, current_millis):
        """
        ...
        current_millis(int): Exact time in milliseconds at which the plugin started execution.
        """
        ...
        web_app_without_replay = self.request(f'{self.tenant_id}/{METRIC_ENDPOINT}?Api-Token={self.token}&metricSelector=builtin:billing.apps.web.sessionsWithoutReplayByApplication&from={self.last_millis-3600000}&to={current_millis-3600000}').json()
        web_app_with_replay = self.request(f'{self.tenant_id}/{METRIC_ENDPOINT}?Api-Token={self.token}&metricSelector=builtin:billing.apps.web.sessionsWithReplayByApplication&from={self.last_millis-3600000}&to={current_millis-3600000}').json()
        web_app_properties = self.request(f'{self.tenant_id}/{METRIC_ENDPOINT}?Api-Token={self.token}&metricSelector=builtin:billing.apps.web.userActionPropertiesByApplication&from={self.last_millis-3600000}&to={current_millis-3600000}').json()
        custom_app_sessions = self.request(f'{self.tenant_id}/{METRIC_ENDPOINT}?Api-Token={self.token}&metricSelector=builtin:billing.apps.custom.sessionsWithoutReplayByApplication&from={self.last_millis-3600000}&to={current_millis-3600000}').json()
        custom_app_properties = self.request(f'{self.tenant_id}/{METRIC_ENDPOINT}?Api-Token={self.token}&metricSelector=builtin:billing.apps.custom.userActionPropertiesByDeviceApplication&from={self.last_millis-3600000}&to={current_millis-3600000}').json()
        mobile_app_without_replay = self.request(f'{self.tenant_id}/{METRIC_ENDPOINT}?Api-Token={self.token}&metricSelector=builtin:billing.apps.mobile.sessionsWithoutReplayByApplication&from={self.last_millis-3600000}&to={current_millis-3600000}').json()
        mobile_app_properties = self.request(f'{self.tenant_id}/{METRIC_ENDPOINT}?Api-Token={self.token}&metricSelector=builtin:billing.apps.mobile.userActionPropertiesByMobileApplication&from={self.last_millis-3600000}&to={current_millis-3600000}').json()
        mobile_app_with_replay = self.request(f'{self.tenant_id}/{METRIC_ENDPOINT}?Api-Token={self.token}&metricSelector=builtin:billing.apps.mobile.sessionsWithReplayByApplication&from={self.last_millis-3600000}&to={current_millis-3600000}').json()
        synthetic_actions = self.request(f'{self.tenant_id}/{METRIC_ENDPOINT}?Api-Token={self.token}&metricSelector=builtin:billing.synthetic.actions&from={self.last_millis-3600000}&to={current_millis-3600000}').json()
        synthetic_requests = self.request(f'{self.tenant_id}/{METRIC_ENDPOINT}?Api-Token={self.token}&metricSelector=builtin:billing.synthetic.requests&from={self.last_millis-3600000}&to={current_millis-3600000}').json()
        synthetic_external = self.request(f'{self.tenant_id}/{METRIC_ENDPOINT}?Api-Token={self.token}&metricSelector=builtin:billing.synthetic.external&from={self.last_millis-3600000}&to={current_millis-3600000}').json()
    ...
    def add_entities(self, entity_dictionary, entity_type):
        ...
        if entity_type == 'DYNAMO_DB_TABLE':
            entity_api_response = self.request(f'{self.tenant_id}/{ENTITY_ENDPOINT}?Api-Token={self.token}&pageSize=4000&entitySelector=type("{entity_type}")&from=1&fields=toRelationships,tags').json()
        ...
        # EBS_VOLUME do not have a managementZones value, so we use the one of the EC2_INSTANCE where they belong
            ...
            entity_api_response = self.request(f'{self.tenant_id}/{ENTITY_ENDPOINT}?Api-Token={self.token}&pageSize=4000&entitySelector=type("{entity_type}")&from=1&fields=fromRelationships,tags').json()
            entity_list = entity_api_response.get('entities', [])
            for entity in entity_list:
        ...
        # Generic for anything else
            ...
            entity_api_response = self.request(f'{self.tenant_id}/{ENTITY_ENDPOINT}?Api-Token={self.token}&pageSize=4000&entitySelector=type("{entity_type}")&from=1&fields=managementZones,tags').json()
            entity_list = entity_api_response.get('entities', [])
            for entity in entity_list:
                entity_dictionary[entity.get('entityId', '')] = {}
    ...
    def calculate_and_push_consumption_for_ddu(self, entity_definitions, current_millis, hosts):
        """
        ...
        """
        ddu_consumption = {}
        ddu_per_entity = self.request(f'{self.tenant_id}/{METRIC_ENDPOINT}?Api-Token={self.token}&metricSelector=builtin:billing.ddu.metrics.byEntity&from={self.last_millis-180000}&to={current_millis-180000}').json()
            ...
            if entity_id:
                ...
                if entity_id not in entity_definitions:
                    if entity_id.split('-')[0] == "HOST":
                        ...
                        if int(time.time() * 1000) - current_millis < 30000:
                            logger.info("Fetch " + entity_id.split('-')[0])
                            self.add_entities(entity_definitions, entity_id.split('-')[0])
                            logger.info("Fetched " + entity_id.split('-')[0])
                        else:
                            logger.info("No time to fetch " + entity_id.split('-')[0])
